module.exports = {
    ...
    description: "Help function for rob command with usage and info",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Rob Help`, user.displayAvatarURL())
        .setTitle('Rob command')
        .setURL('https://crizmo.github.io/elina/')
        .setDescription('Earnings method - rob')
        .addFields(
            {name: 'Aliases: ', value: "rob"},
            {name: 'Usage: ', value: "=rob {user}"},
            {name: 'Info: ', value: "You can rob every 5 mins , Min amount one can get from gambling is 100 ¥ & Max is 1000 ¥"},
        )
