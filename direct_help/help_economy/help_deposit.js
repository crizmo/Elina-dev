module.exports = {
    ...
    description: "Help function for deposit command with usage",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Deposit Help`, user.displayAvatarURL())
        .setTitle('Deposit command')
        .setURL('https://dsc.gg/elina')
        .setDescription('deposit money in your bank')
        .addFields(
            {name: 'Aliases: ', value: "dep"},
            {name: 'Usage: ', value: "=deposit {amount of cash} "},
        )
