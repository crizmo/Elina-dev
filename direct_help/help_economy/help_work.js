module.exports = {
    ...
    description: "Help function for work command with usage",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Work Help`, user.displayAvatarURL())
        .setTitle('Work command')
        .setURL('https://crizmo.github.io/elina/')
        .setDescription('Earnings method - working')
        .addFields(
            {name: 'Aliases: ', value: "work"},
            {name: 'Usage: ', value: "=work"},
            {name: 'Info: ', value: "You can work ever 60 seconds , maximum amount one can get from working is 500 ¥"},
        )
