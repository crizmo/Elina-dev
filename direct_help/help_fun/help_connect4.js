module.exports = {
    name: 'help-connectfour',
    permissions: ["SEND_MESSAGES"],
    description: "Help function for connectfour command with usage",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Connectfour Help`, user.displayAvatarURL())
        .setTitle('Connectfour command')
        .setURL('https://dsc.gg/elina')
        .setDescription('Connectfour command with usage')
        .addFields(
            {name: 'Aliases: ', value: ['c4','connectfour','connect4']},
            {name: 'Usage: ', value: "=connectfour | =c4 @anothermember"},
        )
