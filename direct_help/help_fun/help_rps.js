module.exports = {
    ...
    description: "Help function for rps command with usage",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Rps Help`, user.displayAvatarURL())
        .setTitle('Rps command')
        .setURL('https://dsc.gg/elina')
        .setDescription('Rock Papaer Scissors Command')
        .addFields(
            {name: 'Aliases: ', value: "rps"},
            {name: 'Usage: ', value: "=rps @Criz"},
        )
