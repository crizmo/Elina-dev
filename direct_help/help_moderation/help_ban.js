module.exports = {
    ...
    description: "Help function for ban command with usage",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Ban Help`, user.displayAvatarURL())
        .setTitle('Ban command')
        .setURL('https://dsc.gg/elina')
        .setDescription('Ban command with usage')
        .addFields(
            {name: 'Aliases: ', value: "ban"},
            {name: 'Usage: ', value: "=ban {usermention}"},
        )
