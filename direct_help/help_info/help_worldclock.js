module.exports = {
    name: 'help-worldclock',
    permissions: ["SEND_MESSAGES"],
    description: "Help function for worldclock command with usage",
    execute(client, message, args, Discord){
        const user = message.mentions.users.first() || message.member.user
        const newEmbed = new Discord.MessageEmbed()

        .setColor('#FFDBE9')
        .setAuthor(`Worldclock Help`, user.displayAvatarURL())
        .setTitle('Worldclock command')
        .setURL('https://dsc.gg/elina')
        .setDescription('Get worldclock stats')
        .addFields(
            {name: 'Aliases: ', value: `"wc","clock"`},
            {name: 'Usage: ', value: "=worldclock | =wc | =clock"},
        )
